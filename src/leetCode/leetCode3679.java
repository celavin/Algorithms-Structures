...
public class leetCode3679 {
    public static void main(String[] args) {

    }
    public int minArrivalsToDiscard(int[] arrivals, int w, int m) {
        boolean[] discard  = new boolean[arrivals.length];
        int left = 0;
        int right = 0;
        int ans = 0;
        HashMap<Integer,Integer> hm = new HashMap<>();
        while(right<arrivals.length){
            if(hm.getOrDefault(arrivals[right],0)+1<=m) {
                hm.put(arrivals[right], hm.getOrDefault(arrivals[right], 0) + 1);
            }else {
                discard[right] = true;
                ans++;
            }
            right++;
            while(right-left>=w){
                if(discard[left]==false) {
                    hm.put(arrivals[left], hm.get(arrivals[left]) - 1);
                }
                left++;
            }
